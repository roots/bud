...
[bud.assets](/docs/bud.assets) allows you to include static assets in your build.

You can specify a path to a specific file or use glob syntax to match many files at once.

## Signature

```ts title='bud.assets.d.ts'
type Assets = (
  this: Framework
  paths: string[]
) => Framework
...
Copy only gifs and jpgs found in **src**:

```js title='bud.config.js'
bud.assets(['src/**/*.{jpg,gif}'])
```
