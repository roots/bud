...
describe('issue-1886', () => {
  ...
  it('should work with disk caching', async () => {
    await execa(`yarn`, [`bud`, `clean`], {
      cwd: join(paths.tests, `reproductions`, `issue-1886`),
    })

    const res1 = await execa(`yarn`, [`bud`, `build`], {
      cwd: join(paths.tests, `reproductions`, `issue-1886`),
    })
    const res2 = await execa(`yarn`, [`bud`, `build`], {
      cwd: join(paths.tests, `reproductions`, `issue-1886`),
    })
    const res3 = await execa(`yarn`, [`bud`, `build`], {
      cwd: join(paths.tests, `reproductions`, `issue-1886`),
    })
