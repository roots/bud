import {Hooks as Base} from '@roots/bud-hooks'
import {Service} from '@roots/bud-framework'
import {source} from '../store/source'

export class Hooks extends Base implements Service {
  public name = 'service/hooks'

  public register() {
    this.setStore({
      'loader/css': [],
      'loader/cache': [],
      'loader/file': [],
      'loader/minicss': [],
      'loader/raw': [],
      'loader/resolve-url': [],
      'loader/style': [],
      'loader/thread': [],
      item: [],
      'item/css': [],
      'item/css/loader': [],
      'item/css/options': [],
      'item/css/options/sourceMap': [],
      'item/cache': [],
      'item/cache/loader': [],
      'item/cache/options': [],
      'item/cache/options/cacheDirectory': [],
      'item/file': [],
      'item/file/loader': [],
      'item/file/options': [],
      'item/file/options/name': [],
      'item/js': [],
      'item/js/loader': [],
      'item/js/options': [],
      'item/minicss': [],
      'item/minicss/loader': [],
      'item/minicss/options': [],
      'item/minicss/options/publicPath': [],
      'item/resolve-url': [],
      'item/resolve-url/loader': [],
      'item/resolve-url/options': [],
      'item/raw': [],
      'item/raw/loader': [],
      'item/style': [],
      'item/style/loader': [],
      'item/svg': [],
      'item/svg/loader': [],
      'item/svg/options': [],
      'item/thread': [],
      'item/thread/loader': [],
      rule: [],
      'rule/css': [],
      'rule/css/test': [],
      'rule/css/exclude': [],
      'rule/css/use': [],
      'rule/css/root': [],
      'rule/font': [],
      'rule/font/test': [],
      'rule/font/exclude': [],
      'rule/font/use': [],
      'rule/html': [],
      'rule/html/test': [],
      'rule/html/use': [],
      'rule/image': [],
      'rule/image/use': [],
      'rule/image/test': [],
      'rule/js': [],
      'rule/js/test': [],
      'rule/js/exclude': [],
      'rule/js/use': [],
      'rule/svg': [],
      'rule/svg/test': [],
      'rule/svg/exclude': [],
      'rule/svg/use': [],
      build: [],
      'build/bail': [],
      'build/cache': [],
      'build/cache/name': [],
      'build/cache/location': [],
      'build/cache/directory': [],
      'build/cache/type': [],
      'build/context': [],
      'build/devtool': [],
      'build/entry': [],
      'build/externals': [],
      'build/infrastructureLogging': [],
      'build/mode': [],
      'build/module': [],
      'build/module/rules': [],
      'build/module/rules/oneOf': [],
      'build/module/rules/parser': [],
      'build/name': [],
      'build/node': [],
      'build/output': [],
      'build/output/filename': [],
      'build/output/path': [],
      'build/output/publicPath': [],
      'build/optimization': [],
      'build/optimization/namedModules': [],
      'build/optimization/noEmitOnErrors': [],
      'build/optimization/minimize': [],
      'build/optimization/minimizer': [],
      'build/optimization/runtimeChunk': [],
      'build/optimization/splitChunks': [],
      'build/optimization/splitChunks/cacheGroups': [],
      'build/optimization/splitChunks/cacheGroups/vendor': [],
      'build/parallelism': [],
      'build/performance': [],
      'build/plugins': [],
      'build/profile': [],
      'build/recordsPath': [],
      'build/resolve': [],
      'build/resolve/alias': [],
      'build/resolve/extensions': [],
      'build/resolve/modules': [],
      'build/resolve/': [],
      'build/stats': [],
      'build/target': [],
      'build/watch': [],
      'build/watchOptions': [],
      'location/project': [
        () => source(['location.project', 'APP_PATH']),
      ],

      'location/src': [
        () => source(['location.src', 'APP_SRC']),
      ],
      'location/dist': [
        () => source(['location.dist', 'APP_DIST']),
      ],
      'location/storage': [
        () => source(['location.storage', 'APP_STORAGE']),
      ],
      'location/modules': [
        () => source(['location.modules', 'APP_MODULES']),
      ],
      'location/publicPath': [
        () => source(['location.publicPath', 'APP_PUBLIC_PATH']),
      ],
      'location/records': [
        () => source(['location.records', 'APP_RECORDS']),
      ],
    })
  }
}
