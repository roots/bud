  ...
  public async execute() {
    ...
    await this.$(
      ...[
        ...(this.eslint
          ? [
              `yarn eslint workspaces/@roots/ --config ./config/eslint.config.js --ignore-path config/.eslintignore`,
            ]
          : []),
        this.skypack
    ...
    if (this.prettier) {
      await this.$(
        ...[
          `yarn prettier ./workspaces/@roots/*/src/**/* --config ./config/prettier.config.js --write --ignore-unknown --no-error-on-unmatched-pattern`,
          this.lib
            ? `yarn prettier ./workspaces/@roots/*/lib/**/* --config ./config/prettier.config.js --write --ignore-unknown --no-error-on-unmatched-pattern`
            : null,
          this.types
            ? `yarn prettier ./workspaces/@roots/*/types/**/*.d.ts --config ./config/prettier.config.js --write --ignore-unknown --no-error-on-unmatched-pattern`
            : null,
        ].filter(Boolean),
      )
    }
