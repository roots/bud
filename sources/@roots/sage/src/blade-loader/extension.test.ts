...
describe(`@roots/sage/blade-loader`, () => {
  ...
  it(`should register blade-loader (loader)`, async () => {
    ...
    expect(setLoaderSpy).toHaveBeenCalledWith(`@roots/blade-loader`, await bud.module.resolve(`@roots/blade-loader`))
  })
  ...
  it(`should register blade-loader (item)`, async () => {
    ...
    await extension.register(bud)
    expect(setItemSpy).toHaveBeenCalledWith(`@roots/blade-loader`, {loader: `@roots/blade-loader`})
  })
  ...
  it(`should register no-emit (item)`, async () => {
    ...
    const setItemSpy = vi.spyOn(bud.build, `setItem`)
    await extension.register(bud)
    expect(setItemSpy).toHaveBeenCalledWith(`no-emit`, {loader: `file`, options: {emit: false}})
  })
