...
import {alias as aliasFn} from '../src/methods/alias/index.js'
...
describe(`bud.alias`, () => {
  ...
  it(`should be operating in an expected env`, async () => {
    expect(bud.path(`@src`)).toBe(
      repoPath(`sources`, `tests`, `util`, `project`, `src`),
    )
  })
  ...
  describe(`with malformed input`, () => {
    ...
    it(`should error when a non string value is passed`, async () => {
      vi.mock(`../src/methods/alias/isRecords.js`)
      vi.mock(`../src/methods/alias/handleRecords.js`)
      vi.mock(`../src/methods/alias/handleTypeError.js`)
      vi.mock(`../src/methods/alias/isSignifierValuePair.js`)
      ...
      expect(
        await import(`../src/methods/alias/isRecords.js`).then(({isRecords}) => isRecords),
      ).toHaveBeenCalledTimes(1)
      expect(
        await import(`../src/methods/alias/handleRecords.js`).then(
          ({handleRecords}) => handleRecords,
        ),
      ).toHaveBeenCalledTimes(1)

      expect(
        await import(`../src/methods/alias/isSignifierValuePair.js`).then(
    ...
    it(`should error when passed more than two parameters`, async () => {
      vi.mock(`../src/methods/alias/isCallback.js`)
      vi.mock(`../src/methods/alias/isRecords.js`)
      vi.mock(`../src/methods/alias/isSignifierValuePair.js`)
      vi.mock(`../src/methods/alias/handleFallthrough.js`)
      ...
      expect(
        await import(`../src/methods/alias/handleFallthrough.js`).then(
          ({handleFallthrough}) => handleFallthrough,
        ),
      ).toHaveBeenCalledTimes(1)
      expect(
        await import(`../src/methods/alias/isCallback.js`).then(({isCallback}) => isCallback),
      ).toHaveBeenCalledTimes(1)
      expect(
        await import(`../src/methods/alias/isRecords.js`).then(({isRecords}) => isRecords),
      ).toHaveBeenCalledTimes(1)
