...
export class Compile extends Command {
  ...
  public async execute() {
    await this.$(
      `yarn ts-node --project ./config/tsconfig.base.json ./dev/compile/cjs ${this.package}`,
    )

    await this.$(
      `yarn ts-node --project ./config/tsconfig.base.json ./dev/compile/esm ${this.package}`,
    )
  }
}
