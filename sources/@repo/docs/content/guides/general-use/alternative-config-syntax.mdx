...
## Static config (`bud.config.json` and `bud.config.yml`)
...
If using `json` your config file will be parsed with json5. This means you can use comments, trailing commas, amongst other niceties.
...
Config function names are used as keys. Their values serve as function parameters.
