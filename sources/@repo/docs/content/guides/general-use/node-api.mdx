...
If `get` is called again with the same basedir it will return the same instance. This means that you can arbitrarily access
`bud` from anywhere in your project and have assurance you're working with the same `bud` instance.
...
| mode       | `production` or `development`     | no       |
| args       | cli args                          | yes      |
