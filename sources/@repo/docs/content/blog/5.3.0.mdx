...
- There is a base tsconfig available to import from `@roots/bud-typescript/tsconfig/tsconfig.json`. For the msot part this config is identical to the Microsoft recommended tsconfig except that it allows jsx/tsx to be passed through to `@roots/bud-babel`.
