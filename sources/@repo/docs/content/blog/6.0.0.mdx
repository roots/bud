...
You can convert `require` statements to use `import` without worrying about it too much:
...
Importing ESM from CommonJS is a little less straight forward than the other way around.

You'll need to use a dynamic `import` statement:
...
- `@roots/bud-library` is deprecated (just [use the standard cache](/docs/bud.persist) -- it's now better than this extension)
