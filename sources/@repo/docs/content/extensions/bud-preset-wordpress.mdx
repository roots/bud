...
You can register more than one type in a single entrypoint:
...
export const callback = (settings, name) => {
  ...
  settings.example.attributes.values = `<li>👋</li>`
  return settings;
}
```
...
## Disabling snackbar notification(s)

There is a snackbar message when the hot reload fires for the first time. There will probably be a couple more in the future (errors, warnings).

Disable snackbar notices:

```js title=bud.config.js
bud.wp.set(`notify`, false)
```
