...
image: 'https://bud.js.org/img/share/extensions/bud-babel.png'
---
...
As a courtesy, a base configuration is provided in `@roots/bud-babel/eslint-config`.

You may use it in conjunction with `@roots/bud-eslint/config` for a complete linting configuration.

```js
module.exports = {
  root: true,
  extends: [
    require.resolve('@roots/bud-eslint/config'),
