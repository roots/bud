...
[Read the official announcement on bud.js.org](https://bud.js.org/blog/6.0.0).
...
The `node:module` interface provides a function `createRequire` that will let you directly require `CommonJS` code like you may be used to. [See the nodejs docs on `createRequire`](https://nodejs.org/api/module.html#modulecreaterequirefilename).
...
This functionality is provided using the [`import-meta-resolve`](https://github.com/wooorm/import-meta-resolve) package. In the future we'll use the `import.meta.resolve` API directly but right now it is labeled as `experimental` and requires a flag.
