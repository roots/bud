...
We're using the `nodenext` tsc `module` setting. This enables top-level `await` for core bud packages 🥳.

### 🧪 test: migrate jest to vitest (#1850)

We switched from jest to vitest and it's pretty cool. Hopefully we'll be able to run unit tests on the CLI now.
